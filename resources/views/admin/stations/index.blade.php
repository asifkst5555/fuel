            ...
            <div class="insight-card">
                ...
                <div class="row g-3">
                    @forelse ($topStations as $index => $topStation)
                        <div class="col-md-6"><div class="summary-card"><div class="summary-label">Rank #{{ $index + 1 }}</div><div class="fw-bold mb-1">{{ $topStation->name }}</div><div class="small text-secondary mb-2">{{ $topStation->location }}</div><div class="summary-value text-primary" style="font-size:1.5rem;">{{ $topStation->crowd_reports_count }}</div><div class="small text-secondary">public crowd reports</div></div></div>
                    @empty
        ...
        <div class="col-xl-6">
            <div class="insight-card">
                ...
                @forelse ($staleStations as $staleStation)
                    <div class="activity-item">
                        <div><div class="fw-bold">{{ $staleStation->name }}</div><div class="small text-secondary">{{ $staleStation->location }}</div></div>
        ...
        <div class="panel-header">
            <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                ...
                <form method="GET" action="{{ route('admin.stations.index') }}" class="d-flex gap-2 flex-wrap w-100" style="max-width: 420px;">
                    <input type="text" name="search" value="{{ $search }}" class="form-control rounded-4 flex-grow-1" placeholder="Search station or location">
        ...
        <div class="panel-body pt-4">
            @if ($stations->count())
                <div class="table-responsive desktop-table">
                    <table class="table align-middle">
                        <thead><tr><th>Station</th><th>Location</th><th>Fuel Status</th><th>Last Updated</th><th class="text-end">Actions</th></tr></thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="min-width: 320px;">
                                        <form method="POST" action="{{ route('admin.stations.status', $station) }}" class="row g-2 align-items-end">
                                            @csrf @method('PATCH')
                                            <div class="col-12 col-md-5"><label class="form-label small text-secondary fw-semibold mb-1">Octane</label><select name="octane" class="form-select form-select-sm rounded-4 status-select"><option value="1" @selected($status?->octane)>Available</option><option value="0" @selected(! $status?->octane)>Not Available</option></select></div>
                                            <div class="col-12 col-md-5"><label class="form-label small text-secondary fw-semibold mb-1">Diesel</label><select name="diesel" class="form-select form-select-sm rounded-4 status-select"><option value="1" @selected($status?->diesel)>Available</option><option value="0" @selected(! $status?->diesel)>Not Available</option></select></div>
                                            <div class="col-12 col-md-2 d-flex align-items-end"><button type="submit" class="btn btn-sm btn-primary rounded-4 w-100">Save</button></div>
                                        </form>
                                    </td>
                ...
                <div class="mobile-station-list">
                    @foreach ($stations as $station)
                        ...
                        <div class="mobile-station-card">
                            <div class="d-flex justify-content-between align-items-start gap-3"><div><div class="fw-bold fs-5">{{ $station->name }}</div><div class="small text-secondary mt-1">{{ $station->location }}</div></div><span class="station-tag">ID {{ $station->id }}</span></div>
                            ...
                            <form method="POST" action="{{ route('admin.stations.status', $station) }}" class="row g-2 mt-1">
                                @csrf @method('PATCH')
                                <div class="col-6"><label class="form-label small text-secondary fw-semibold mb-1">Octane</label><select name="octane" class="form-select form-select-sm rounded-4"><option value="1" @selected($status?->octane)>Available</option><option value="0" @selected(! $status?->octane)>Not Available</option></select></div>
                                <div class="col-6"><label class="form-label small text-secondary fw-semibold mb-1">Diesel</label><select name="diesel" class="form-select form-select-sm rounded-4"><option value="1" @selected($status?->diesel)>Available</option><option value="0" @selected(! $status?->diesel)>Not Available</option></select></div>
            ...
            @else
                <div class="empty-state">
                    ...
                    <p class="text-secondary mb-4">{{ $isAdmin ? 'Start by creating a station and setting its octane and diesel availability.' : 'Ask an administrator to assign one or more stations to your account.' }}</p>
                    @if ($isAdmin)
...
@push('scripts')
    ...
    <script>
        (() => {
            ...
            if (fuelCanvas) {
                new Chart(fuelCanvas, { type: 'doughnut', data: { labels: chartData.fuel.labels, datasets: [{ data: chartData.fuel.values, backgroundColor: ['#16a34a', '#2563eb', '#d97706'], borderWidth: 0 }] }, options: { plugins: { legend: { position: 'bottom' } }, cutout: '68%' } });
            }
            if (crowdCanvas) {
